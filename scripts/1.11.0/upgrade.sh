...
#
# TIBCO Control Plane Helm Values Generation Script for Platform Upgrade (1.10.0 → 1.11.0)
...
TP_HELM_CHARTS_REPO=""
PLATFORM_BOOTSTRAP_CHART_VERSION="${PLATFORM_BOOTSTRAP_CHART_VERSION:-1.11.0}"
PLATFORM_BASE_CHART_VERSION="${PLATFORM_BASE_CHART_VERSION:-1.11.0}"
...
NOTE:
  ...
  - It assumes all chart values are correct, performs the upgrade from 1.10.0 to 1.11.0
...
interactive_mode() {
    ...
    while true; do
        ...
        case $choice in
            1)
                print_info "Selected: Generate 1.11.0 values.yaml files"
...
interactive_values_generation() {
    ...
    print_info "======================"
    echo ""
    echo "How would you like to provide your current 1.10.0 values?"
...
interactive_helm_upgrade() {
    ...
    if [[ "${UPGRADE_BOOTSTRAP}" == "true" ]]; then
        while [[ -z "${PLATFORM_BOOTSTRAP_FILE}" ]]; do
            read -p "Enter file name of 1.11.0 platform-bootstrap values.yaml file: " PLATFORM_BOOTSTRAP_FILE
            if [[ ! -f "${PLATFORM_BOOTSTRAP_FILE}" ]]; then
                print_error "File not found: ${PLATFORM_BOOTSTRAP_FILE}"
                PLATFORM_BOOTSTRAP_FILE=""
            fi
        ...
        if [[ "${bs_status}" == "deployed" ]]; then
            if [[ "${bs_version}" =~ ^1\.11\. ]]; then
                if [[ "${UPGRADE_MINOR_VERSIONS}" == "true" ]]; then
                    ...
                    print_warning "This will perform a minor version upgrade within the 1.11.x series"
                    read -p "Do you want to proceed with the 1.11.x minor version upgrade? (yes/no): " proceed_bs
                    if [[ "${proceed_bs}" != "yes" ]]; then
                        print_info "Skipping minor version upgrade for ${PLATFORM_BOOTSTRAP_RELEASE_NAME} per user choice."
                        UPGRADE_BOOTSTRAP=false
                    fi
                else
                    print_error "Current ${PLATFORM_BOOTSTRAP_RELEASE_NAME} version is '${bs_version}', expected '1.10.0'"
                    print_error "This script only supports upgrades from 1.10.0 to 1.11.0"
                    return 1
                fi
            elif [[ "${bs_version}" != "1.10.0" ]]; then
                print_error "Current ${PLATFORM_BOOTSTRAP_RELEASE_NAME} version is '${bs_version}', expected '1.10.0'"
                print_error "This script only supports upgrades from 1.10.0 to 1.11.0"
                return 1
            else
            ...
            if echo " ${FAILED_STATES} " | grep -q " ${bs_status} "; then
                if [[ "${bs_version}" == "1.11.0" ]]; then
                    echo ""
                    print_warning "${PLATFORM_BOOTSTRAP_RELEASE_NAME} is already at app_version 1.11.0 but in '${bs_status}' state."
                    read -p "Do you want to retry the 1.11.0 upgrade for ${PLATFORM_BOOTSTRAP_RELEASE_NAME}? (yes/no): " retry_bs
        ...
        if [[ "${base_status}" == "deployed" ]]; then
            if [[ "${base_version}" =~ ^1\.11\. ]]; then
                if [[ "${UPGRADE_MINOR_VERSIONS}" == "true" ]]; then
                    ...
                    print_warning "This will perform a minor version upgrade within the 1.11.x series"
                    read -p "Do you want to proceed with the 1.11.x minor version upgrade? (yes/no): " proceed_pb
                    if [[ "${proceed_pb}" != "yes" ]]; then
                        print_info "Skipping minor version upgrade for ${PLATFORM_BASE_RELEASE_NAME} per user choice."
                        UPGRADE_BASE=false
                    fi
                else
                    print_error "Current ${PLATFORM_BASE_RELEASE_NAME} version is '${base_version}', expected '1.10.0'"
                    print_error "This script only supports upgrades from 1.10.0 to 1.11.0"
                    print_error "To enable 1.11.x minor version upgrades, set UPGRADE_MINOR_VERSIONS=true"
                    return 1
                fi
            elif [[ "${base_version}" != "1.10.0" ]]; then
                print_error "Current ${PLATFORM_BASE_RELEASE_NAME} version is '${base_version}', expected '1.10.0'"
                print_error "This script only supports upgrades from 1.10.0 to 1.11.0"
                return 1
            else
            ...
            if echo " ${FAILED_STATES} " | grep -q " ${base_status} "; then
                if [[ "${base_version}" == "1.11.0" ]]; then
                    echo ""
                    print_warning "${PLATFORM_BASE_RELEASE_NAME} is already at app_version 1.11.0 but in '${base_status}' state."
                    read -p "Do you want to retry the 1.11.0 upgrade for ${PLATFORM_BASE_RELEASE_NAME}? (yes/no): " retry_pb
...
extract_helm_values() {
    ...
    if [[ "${bs_ver}" != "1.10.0" ]]; then
        print_error "${PLATFORM_BOOTSTRAP_RELEASE_NAME} app_version is '${bs_ver}', expected 1.10.0"
        exit 1
    fi
    if [[ "${base_ver}" != "1.10.0" ]]; then
        print_error "${PLATFORM_BASE_RELEASE_NAME} app_version is '${base_ver}', expected 1.10.0"
        exit 1
    fi
