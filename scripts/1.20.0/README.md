...
# TIBCO Control Plane Upgrade Assistant (1.19.0 ➜ 1.20.0)

This directory contains an interactive Bash script to assist with upgrading the TIBCO Control Plane from version 1.19.0 to 1.20.0.

File: `scripts/1.20.0/upgrade.sh`
...
- Prompts you interactively to:
  - Generate 1.20.0 values file from an existing 1.19.0 deployment or from existing values files
  - Perform Helm upgrade to `tibco-cp-base` chart version 1.20.0
...
- Two operation modes:
  ...
  2) Helm upgrade (uses your 1.20.0-ready values)
...
- Strict version gating:
  - Validates currently deployed app_version is exactly `1.19.0` before values extraction or upgrade
...
- Retry-aware upgrades:
  - If a previous attempt reached `app_version` `1.20.0` but the release is in a failed-like state, the script prompts to retry the 1.20.0 upgrade
- Post-upgrade verification:
  - Confirms chart name ends with `-${CHART_VERSION}` (e.g., `-1.20.0`)
  - Confirms upgraded `app_version` is `1.20.0`
...
- Choose between:
  - Generate values file for 1.20.0
...
Note:
- Post-upgrade `app_version` is validated against `1.20.0` (without pre-release suffix)
...
- Deployed version (pre-upgrade):
  - `app_version` must be exactly `1.19.0` for the control plane release
- Tools availability:
...
- During values extraction from Helm, the script validates the release for existence, `deployed` status, and `app_version == 1.19.0` before extracting values.
- During upgrades, if a release is in a failed-like state but already at `app_version == 1.20.0`, you will be prompted to retry the 1.20.0 upgrade; otherwise, the script blocks and asks you to fix the state (e.g., rollback) first.
...
No version-specific transformations have been defined yet for 1.20.0.

Update the `process_files()` function in `upgrade.sh` once the deprecated fields or new configurations for 1.20.0 are confirmed.
...
```bash
cd scripts/1.20.0 && ./upgrade.sh
```
Select option 1 when prompted.

To test with an alpha/beta chart version:
```bash
export CHART_VERSION="1.20.0-alpha.1" && ./upgrade.sh
```
