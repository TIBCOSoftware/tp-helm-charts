...
#
# TIBCO Control Plane Helm Values Generation Script for Platform Upgrade FROM 1.14.0 TO 1.15.0
...
NOTE:
  ...
  - It assumes all chart values are correct, performs the upgrade from 1.14.0 to 1.15.0
...
interactive_mode() {
    print_info "TIBCO Control Plane Upgrade Script - 1.14.0 to 1.15.0"
    print_info "======================================================="
    echo ""
    print_info "Note: This script works with the unified control plane chart (tibco-cp-base/platform-base)."
    print_info "This script will extract values from your 1.14.0 deployment and upgrade to 1.15.0."
    echo ""
    
    echo "Please select your operation mode:"
    echo "1) Generate 1.15.0 values.yaml file from current 1.14.0 setup"
    echo "2) Perform Helm upgrade using existing 1.15.0-compatible values.yaml file"
    echo ""
    ...
    while true; do
        ...
        case $choice in
            1)
                print_info "Selected: Generate 1.15.0 values.yaml file"
...
interactive_values_generation() {
    ...
    print_info "======================"
    echo ""
    echo "How would you like to provide your current 1.14.0 values?"
    echo "1) I have existing 1.14.0 values.yaml file"
...
interactive_file_input() {
    ...
    while [[ -z "${PLATFORM_BASE_FILE}" ]]; do
        read -p "Enter file name of control plane values.yaml file (1.14.0): " PLATFORM_BASE_FILE
    ...
    read -p "Custom output file for control plane (default: control-plane-1.15.0.yaml): " custom_output
    if [[ -n "${custom_output}" ]]; then
        PLATFORM_OUTPUT_FILE="${custom_output}"
        print_info "Using custom output: ${PLATFORM_OUTPUT_FILE}"
    else
        print_info "Using default output: control-plane-1.15.0.yaml"
    fi
...
interactive_helm_upgrade() {
    ...
    print_info "This mode will perform actual Helm upgrade on your cluster using 1.15.0 values.yaml file."
    echo ""
    
    echo "This will upgrade to tibco-cp-base chart (v1.15.0)"
    echo ""
    
    while [[ -z "${PLATFORM_BASE_FILE}" ]]; do
        read -p "Enter file name of 1.15.0 control plane values.yaml file: " PLATFORM_BASE_FILE
        if [[ ! -f "${PLATFORM_BASE_FILE}" ]]; then
            print_error "File not found: ${PLATFORM_BASE_FILE}"
            PLATFORM_BASE_FILE=""
        fi
...
perform_helm_upgrade() {
    ...
    if [[ "${release_status}" == "deployed" ]]; then
        if [[ "${release_version}" != "1.14.0" && "${release_version}" != "1.15.0" ]]; then
            print_error "Current ${CONTROL_PLANE_RELEASE_NAME} version is '${release_version}', expected '1.14.0' or '1.15.0'"
            print_error "This script supports upgrades from 1.14.0 to 1.15.0"
            return 1
        fi
        print_success "Release version validation passed: ${release_version}"
    else
        if echo " ${FAILED_STATES} " | grep -q " ${release_status} "; then
            if [[ "${release_version}" == "1.15.0" ]]; then
                echo ""
                print_warning "${CONTROL_PLANE_RELEASE_NAME} is already at app_version 1.15.0 but in '${release_status}' state."
                read -p "Do you want to retry the 1.15.0 upgrade for ${CONTROL_PLANE_RELEASE_NAME}? (yes/no): " retry_confirm
...
# (Bootstrap cleanup not needed for 1.15.0 upgrade - no separate bootstrap chart)
# Verify Helm upgrades
verify_helm_upgrades() {
    print_info "Verifying Helm upgrade..."
    echo ""
    # Hard-coded expected appVersion after upgrade
    local EXPECTED_POST_APP_VERSION="1.15.0"
    ...
    print_info "It assumes all chart values are correct, performs the upgrade from 1.14.0 to 1.15.0,"
...
extract_helm_values() {
    ...
    # Validate current deployed app_version is 1.14.0 before extracting values
    ...
    if [[ "${release_ver}" != "1.14.0" ]]; then
        print_error "${CONTROL_PLANE_RELEASE_NAME} app_version is '${release_ver}', expected 1.14.0"
        exit 1
    fi
...
# Main processing function - copies control plane values for 1.15.0
process_files() {
    ...
    # No recipe transformations needed for 1.15.0 upgrade
...
# No recipe transformations needed for 1.15.0 upgrade (no recipe changes)
