# TIBCO Control Plane Upgrade Assistant (1.14.0 ➜ 1.15.0)

This directory contains an interactive Bash script to assist with upgrading the TIBCO Control Plane from version 1.14.0 to 1.15.0.

File: `scripts/1.15.0/upgrade.sh`
...
- Prompts you interactively to:
  - Generate 1.15.0 values file from an existing 1.14.0 deployment or from existing values files
  - Perform Helm upgrade to `tibco-cp-base` chart version 1.15.0
...
- Two operation modes:
  ...
  2) Helm upgrade (uses your 1.15.0-ready values)
...
- Strict version gating:
  - Validates currently deployed app_version is exactly `1.14.0` before values extraction or upgrade
...
- Retry-aware upgrades:
  - If a previous attempt reached `app_version` `1.15.0` but the release is in a failed-like state, the script prompts to retry the 1.15.0 upgrade
- Post-upgrade verification:
  - Confirms chart name ends with `-${CHART_VERSION}` (e.g., `-1.15.0`)
  - Confirms upgraded `app_version` is `1.15.0`
...
- Choose between:
  - Generate values file for 1.15.0
...
Note:
- Post-upgrade `app_version` is validated against `1.15.0` (without pre-release suffix)
...
- Deployed version (pre-upgrade):
  - `app_version` must be exactly `1.14.0` for the control plane release
- Tools availability:
...
- During values extraction from Helm, the script validates the release for existence, `deployed` status, and `app_version == 1.14.0` before extracting values.
- During upgrades, if a release is in a failed-like state but already at `app_version == 1.15.0`, you will be prompted to retry the 1.15.0 upgrade; otherwise, the script blocks and asks you to fix the state (e.g., rollback) first.
...
- It assumes your values files are correct and only:
  - Generates 1.15.0 control plane values (simple copy with minimal changes)
  - Upgrades the chart to `tibco-cp-base` version 1.15.0
...
- **Single chart deployment**: Version 1.14.0+ has only one control plane chart - no bootstrap chart handling needed.
- **No recipe transformations**: The 1.15.0 upgrade does not require recipe transformations.

---

## Simplified Upgrade Process

The 1.15.0 upgrade is simpler than previous upgrades:
...
helm upgrade <your-release-name> tibco-platform/tibco-cp-base \
  --version 1.15.0 \
  --values control-plane-1.15.0.yaml \
...
- "app_version is not 1.14.0": this script upgrades from 1.14.0 to 1.15.0 only; if you're on a different version, use the appropriate upgrade script
- Validation fails for pre-release charts: ensure `CHART_VERSION` matches your chart tag (e.g., `1.15.0-alpha.1`); `app_version` remains `1.15.0`
