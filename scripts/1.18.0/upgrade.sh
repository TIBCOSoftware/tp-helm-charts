...
#
# TIBCO Control Plane Helm Values Generation Script for Platform Upgrade FROM 1.17.0 TO 1.18.0
...
FROM_VERSION="${FROM_VERSION:-1.17.0}"
TO_VERSION="1.18.0"
...
process_files() {
    ...
    # ========================================================================
    # Version-specific transformation logic for 1.17.0 to 1.18.0
    # ========================================================================
    
    # Start with current values as base
    cp "${CONTROL_PLANE_FILE}" "${CONTROL_PLANE_OUTPUT_FILE}"
    
    print_info "Removing deprecated email server configuration fields for 1.18.0..."

    yq eval 'del(.global.external.emailServerType)' -i "${CONTROL_PLANE_OUTPUT_FILE}"
    yq eval 'del(.global.external.fromAndReplyToEmailAddress)' -i "${CONTROL_PLANE_OUTPUT_FILE}"
    yq eval 'del(.global.external.cronJobReportsEmailAlias)' -i "${CONTROL_PLANE_OUTPUT_FILE}"
    yq eval 'del(.global.external.platformEmailNotificationCcAddresses)' -i "${CONTROL_PLANE_OUTPUT_FILE}"
    yq eval 'del(.global.external.emailServer)' -i "${CONTROL_PLANE_OUTPUT_FILE}"

    print_success "Deprecated email server configuration fields removed successfully"
