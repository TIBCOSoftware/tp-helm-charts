...
> **Note**: This master upgrade script has been tested and validated for sequential upgrades across multiple versions (1.10.0 → 1.11.0 → 1.12.0 and so on). The script handles both pre-1.13.0 (separate bootstrap/base charts) and post-1.13.0 (unified tibco-cp-base chart) deployments with intelligent file naming based on version.
...
- Finds all version folders (e.g., `1.10.0/`, `1.11.0/`, `1.12.0/`, and so on)
