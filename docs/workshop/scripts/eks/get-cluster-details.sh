...
export CP_VPC_CIDR=${CP_VPC_CIDR:-"10.180.0.0/16"}
...
_public_subnets=$(aws ec2 describe-subnets --filters "Name=${_tag1},Values=${CP_CLUSTER_NAME}" "Name=${_tag2},Values=1" --query 'Subnets[*].SubnetId' --output json)
...
_private_subnets=$(aws ec2 describe-subnets --filters "Name=${_tag1},Values=${CP_CLUSTER_NAME}" "Name=${_tag2},Values=1" --query 'Subnets[*].SubnetId' --output json)
...
data:
  ...
  CLOUD_REGION: "${CP_CLUSTER_REGION}"
  ### OIDC data
  OIDC_ISSUER_URL: "${OIDC_ISSUER_URL}"
  OIDC_ISSUER_HOSTPATH: "${OIDC_ISSUER_HOSTPATH}"
  OIDC_PROVIDER_ARN: "${OIDC_PROVIDER_ARN}"
  ### Kubernetes data
  KUBERNETES_CLUSTER_NAME: "${CP_CLUSTER_NAME}"
  ### Networking
  NET_VPC_IDENTIFIER: "${VPC_ID}"
  NET_NODE_CIDR: "${CP_VPC_CIDR}"
  NET_PUBLIC_SUBNETS: |
    - "${PUBLIC_SUBNET_ID_1}"
    - "${PUBLIC_SUBNET_ID_2}"
    - "${PUBLIC_SUBNET_ID_3}"
...
[ $? -eq 0 ] || { >&2 tsecho --error "Unable to apply Platform ConfigMap to Kubernetes cluster ${CP_CLUSTER_NAME}."; exit 1; }
