...
{{- $cpCpEnv := printf "%s" "{\"cp\":{\"cp-env\": \"skip\"}}" | fromYaml -}}
{{- if .Values.useCpEnv -}}
  {{/* use cp-env lookup functions to get CP values */}}
  {{- $cpCpEnv = include "cp.values.lookup" . | fromYaml -}}
{{- end -}}
{{- $cpGlobalValues := include "cp.values.global.dig" . | fromYaml -}}
{{- merge $cpGlobalValues $cpCpEnv | toYaml -}}
{{- end  -}}

{{- define "cp.values.global.dig" -}}
