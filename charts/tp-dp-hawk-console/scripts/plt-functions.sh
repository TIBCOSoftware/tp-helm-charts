...
function prometheusApiPath {
    if [ "MSG_DP_TYPE" = "standalone" ] ; then
        export promPath="http://localhost:9090"
    elif [ "MSG_DP_TYPE" = "control-plane" ] ; then
        export promPath="http://prometheus-service:9090"
    else
        export promPath="http://localhost:9090"
    fi
    echo "$promPath"
...
function hawkCTPath {
    ...
    if [ "$MSG_DP_TYPE" = "standalone" ] ; then
        echo >^2 "No Hawk in Standalone"
    elif [ "$MSG_DP_TYPE" = "control-plane" ] ; then
        export hawkCTPath="https://dp-proxy.$MY_NAMESPACE.svc.cluster.local/tibco/agent/infra"
    elif [ "$MSG_DP_TYPE" = "legacy-cp-hosted" ] ; then
        export hawkCTPath="http://dp-$myDataplane.$MY_NAMESPACE.svc.cluster.local:80/tibco/agent/infra"
    elif [ "$MSG_DP_TYPE" = "legacy-cp-saas" ] ; then
        # Need subscription specific namespace
        echo >&2 "hawkCTPath: legacy-cp-saas not supported yet."
    elif [ "$MSG_DP_TYPE" = "control-tower" ] ; then
        export hawkCTPath="https://$(hawkHP)"
    elif [ "$MSG_DP_TYPE" = "k8s" ] ; then
        export hawkCTPath="https://$(hawkHP)"
...
function restdCTPath {
    ...
    if [ "$MSG_DP_TYPE" = "standalone" ] ; then
        ...
        export restdCTPath="http://dp-proxy.$MY_NAMESPACE.svc.cluster.local/tibco/agent/msg/dp/restd"
    elif [ "$MSG_DP_TYPE" = "legacy-cp-hosted" ] ; then
        export restdCTPath="http://dp-$myDataplane.$MY_NAMESPACE.svc.cluster.local/tibco/agent/msg/ct/restd"
    elif [ "$MSG_DP_TYPE" = "legacy-cp-saas" ] ; then
        # Need subscription specific namespace
        echo >&2 "restdCTPath: legacy-cp-saas not supported yet."
    elif [ "$MSG_DP_TYPE" = "control-tower" ] ; then
        export restdCTPath="http://localhost:9014"
    elif [ "$MSG_DP_TYPE" = "k8s" ] ; then
        export restdCTPath="http://localhost:9014"
...
function gatewayApiPath {
    [ -n "$gatewayApiPath" ] && echo "$gatewayApiPath" && return 0
    if [ "$MSG_DP_TYPE" = "standalone" ] ; then
        export gatewayApiPath="http://localhost:7515"
    elif [ "$MSG_DP_TYPE" = "control-plane" ] ; then
        export gatewayApiPath="http://dp-proxy.$MY_NAMESPACE.svc.cluster.local/tibco/agent/msg/dp/ops"
    elif [ "$MSG_DP_TYPE" = "legacy-cp-hosted" ] ; then
        export gatewayApiPath="http://dp-$myDataplane.$MY_NAMESPACE.svc.cluster.local/tibco/agent/msg/ct/toolset"
    elif [ "$MSG_DP_TYPE" = "legacy-cp-saas" ] ; then
        # Need subscription specific namespace
        echo >&2 "gatewayApiPath: legacy-cp-saas not supported yet."
    elif [ "$MSG_DP_TYPE" = "control-tower" ] ; then
        export gatewayApiPath="http://localhost:8376"
    elif [ "$MSG_DP_TYPE" = "k8s" ] ; then
        export gatewayApiPath="http://localhost:8376"
    elif [ -n "$DP_ADMIN_USER" ] ; then
        export gatewayApiPath="http://localhost:8376"
...
function restdSetStdOptions {
  ...
  if [ "$MSG_DP_TYPE" = "control-plane" ] ; then
    useHost="dp-proxy.$MY_NAMESPACE.svc.cluster.local"
  else
    useHost="dp-${myDataplane}.platform.local"
  fi
  export restdCurlOpts=()
...
function gatewayMetricHealth {
    apiPath="/dp/metric/health"
    action="GET"
    gwPath="$(gatewayApiPath)"
    cOpts+=("-H")
    cOpts+=("Host: dp-${myDataplane}.platform.local")
    # restdSetStdOptions
    echo >&2 '#+: curl -i "${cOpts[@]}" ' -X "$action"  "${gwPath}${apiPath}" '| tee curl.out' 
    curl -i "${cOpts[@]}" -X "$action"  "${gwPath}${apiPath}" | tee curl.out >&2
    echo >&2 ; echo >&2 ===
    tail -1 curl.out | jq '.'
    return $rtc
}
