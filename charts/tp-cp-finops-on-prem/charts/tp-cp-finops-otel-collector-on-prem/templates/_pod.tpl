{{- define "tp-cp-finops-otel-collector-on-prem.pod" -}}
{{- with .Values.imagePullSecrets }}
imagePullSecrets:
  {{- toYaml . | nindent 2 }}
{{- end }}
serviceAccountName: {{ include "tp-cp-finops-otel-collector-on-prem.serviceAccountName" . }}
securityContext:
  {{- toYaml .Values.podSecurityContext | nindent 2 }}
containers:
  - name: {{ include "tp-cp-finops-otel-collector-on-prem.lowercase_chartname" . }}
    command:
      - /{{ .Values.command.name }}
    ...
    {{- else }}
    image: "{{ include "tp-cp-finops-otel-collector-on-prem.image.registry" .}}{{"/"}}{{ include "tp-cp-finops-otel-collector-on-prem.image.repository" .}}{{"/"}}opentelemetry-collector-contrib:{{ .Values.image.tag | default .Chart.AppVersion }}"
    {{- end }}
    imagePullPolicy: {{ .Values.image.pullPolicy }}
    
    {{- $ports := include "tp-cp-finops-otel-collector-on-prem.podPortsConfig" . }}
    {{- if $ports }}
    ports:
      {{- $ports | nindent 6}}
    {{- end }}
    env:
      ...
      - name: GOMEMLIMIT
        value: {{ div (mul (include "tp-cp-finops-otel-collector-on-prem.convertMemToMib" .Values.resources.limits.memory) 80) 100 }}MiB
      {{- end }}
    ...
    volumeMounts:
      {{- if .Values.configMap.create }}
      - mountPath: /conf
        name: {{ include "tp-cp-finops-otel-collector-on-prem.lowercase_chartname" . }}-configmap
      {{- end }}
...
volumes:
  ...
  - name: {{ include "tp-cp-finops-otel-collector-on-prem.lowercase_chartname" . }}-configmap
    configMap:
      name: {{ include "tp-cp-finops-otel-collector-on-prem.fullname" . }}{{ .configmapSuffix }}
      items:
        - key: relay
          path: relay.yaml
  {{- end }}
